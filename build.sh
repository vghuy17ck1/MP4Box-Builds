...
valid_target "$target" || die "usage: build.sh <linux64|linuxarm64|win64|winarm64> <minimal|full|shared-minimal|shared-full> <release|master>"
valid_variant "$variant" || die "usage: build.sh <target> <minimal|full|shared-minimal|shared-full> <release|master>"
...
variant_setup
target_setup
BASE_VARIANT="${BASE_VARIANT:-$(variant_base "$variant")}"
LINKAGE="${LINKAGE:-$(variant_linkage "$variant")}"
export BASE_VARIANT LINKAGE
...
printf 'zlib_ref=%s\n' "${zlib_commit:-unknown}" >>"$BUILD_ROOT/refs.env"
compiler_version="$("$CC" --version 2>/dev/null | head -n 1 | tr ' ' '_' | tr -cd '[:alnum:]._+-')"
printf 'compiler_version=%s\n' "$compiler_version" >>"$BUILD_ROOT/refs.env"
source "$ROOT_DIR/scripts.d/00-toolchain.sh"
configure_toolchain "$target" "$PREFIX"
source "$ROOT_DIR/scripts.d/10-zlib.sh"
zlib_ready=false
if [[ "$LINKAGE" == static && -f "$PREFIX/lib/libz.a" ]]; then zlib_ready=true; fi
if [[ "$LINKAGE" == shared ]] && find "$PREFIX/lib" "$PREFIX/bin" -maxdepth 1 -type f \( -name 'libz.so*' -o -iname 'zlib*.dll' -o -iname 'libz*.dll' \) -print -quit | grep -q .; then zlib_ready=true; fi
if [[ "$zlib_ready" != true ]]; then
    zlib_source="$SOURCE_ROOT/zlib"
    bash "$ROOT_DIR/util/download.sh" clone "${ZLIB_REPO:-https://github.com/madler/zlib.git}" "${zlib_commit:?}" "$zlib_source" >"$BUILD_LOG_DIR/zlib-fetch.log"
    build_zlib "$zlib_source" "$PREFIX" >"$BUILD_LOG_DIR/zlib-build.log" 2>&1
fi
source "$ROOT_DIR/scripts.d/40-codec-dependencies.sh"
build_codec_dependencies "$BASE_VARIANT"
...
build_ffmpeg "$ffmpeg_source" "$PREFIX" "$BASE_VARIANT"
...
build_gpac "$gpac_source" "$PREFIX" "$target" "$BASE_VARIANT"
bash "$ROOT_DIR/tests/verify-gpac-tls-policy.sh" "$gpac_source"
bash "$ROOT_DIR/util/verify-prefix.sh" "$PREFIX" "$LINKAGE"
...
if [[ "$LINKAGE" == shared ]]; then
    if [[ "$TARGET_OS" == mingw32 ]]; then
        while IFS= read -r -d '' runtime_file; do
            cp "$runtime_file" "$staging/bin/"
        done < <(find "$PREFIX/bin" "$PREFIX/lib" -type f \( -iname '*.dll' \) -print0)
    else
        while IFS= read -r -d '' runtime_file; do
            relative_file="${runtime_file#"$PREFIX/lib/"}"
            mkdir -p "$staging/lib/$(dirname "$relative_file")"
            cp "$runtime_file" "$staging/lib/$relative_file"
        done < <(find "$PREFIX/lib" \( -type f -o -type l \) \( -name '*.so' -o -name '*.so.*' \) -print0)
        need_cmd patchelf
        patchelf --set-rpath '$ORIGIN/../lib' "$staging/bin/MP4Box" "$staging/bin/gpac"
        while IFS= read -r -d '' runtime_file; do
            case "$runtime_file" in
                */gpac/*) patchelf --set-rpath '$ORIGIN/..' "$runtime_file" ;;
                *) patchelf --set-rpath '$ORIGIN' "$runtime_file" ;;
            esac
        done < <(find "$staging/lib" -type f \( -name '*.so' -o -name '*.so.*' \) -print0)
    fi
fi
...
generate_feature_manifest "$staging/bin" "$staging/FEATURES.json" "$BASE_VARIANT" "$suffix"
...
if bash "$ROOT_DIR/tests/verify-linkage.sh" "$staging" "$target" "$variant"; then static_status=true; fi
...
if [[ "$BASE_VARIANT" == full ]] && bash "$ROOT_DIR/tests/verify-transcode.sh" "$staging" "$target"; then transcode_status=true; fi
if bash "$ROOT_DIR/tests/verify-features.sh" "$staging" "$BASE_VARIANT"; then format_status=true; fi
...
[[ "$static_status" == true ]] || die "linkage and binary-format verification failed for $target/$variant"
...
if [[ -d "$staging/lib" ]]; then cp -R "$staging/lib" "$OUTPUT_DIR/"; fi
cp "$staging/BUILD_INFO.json" "$staging/FEATURES.json" "$OUTPUT_DIR/"
printf 'artifact=%s\n' "$archive"
printf 'linkage_verification=%s\n' "$static_status"

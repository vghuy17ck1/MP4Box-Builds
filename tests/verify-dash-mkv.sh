...
"$gpac" -i "$fixture" -o "$output_dir/manifest.mpd:segdur=2"
...
grep -Eiq 'matroska|webm' <("$gpac" -hx 'ffdmx:*' 2>&1) || { printf 'final gpac does not report FFmpeg Matroska/WebM demuxers\n' >&2; exit 1; }

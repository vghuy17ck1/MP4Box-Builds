...
generate_feature_manifest() {
    ...
    filters="$($gpac -hx filters 2>&1 || true)"
    modules="$($gpac -h modules 2>&1 || true)"
    formats="$($gpac -hx formats 2>&1 || true)"
    ffdmx="$($gpac -hx 'ffdmx:*' 2>&1 || true)"
    ffdec="$($gpac -hx ffdec 2>&1 || true)"
    ffenc="$($gpac -hx ffenc 2>&1 || true)"
    ffbsf="$($gpac -hx ffbsf 2>&1 || true)"
    ffavf="$($gpac -hx ffavf 2>&1 || true)"

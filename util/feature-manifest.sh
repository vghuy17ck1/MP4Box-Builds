...
generate_feature_manifest() {
    ...
    local filters modules ffdmx ffdec ffenc ffbsf ffavf
    filters="$($gpac -h filters 2>&1 || true)"
    modules="$($gpac -h 2>&1 || true)"
    ffdmx="$($gpac -h ffdmx 2>&1 || true)"
    ffdec="$($gpac -h ffdec 2>&1 || true)"
    ffenc="$($gpac -h ffenc 2>&1 || true)"
    ffbsf="$($gpac -h ffbsf 2>&1 || true)"
    ffavf="$($gpac -h ffavf 2>&1 || true)"
...
data = {
    ...
    "cmaf_support": yes(filters, "dasher") and yes(modules, "cmaf"),
